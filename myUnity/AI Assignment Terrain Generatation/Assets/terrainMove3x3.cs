...
public class terrainMove3x3 : MonoBehaviour {

    List<GameObject> terrainQueue = new List<GameObject>();
    Queue<GameObject> terrainQ = new Queue<GameObject>();
    public GameObject theCube = new GameObject();
    Rect rect;
        ...
        for (int w = 0; w < 9; w++)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < (int)(size / frequency); j++)
                    ...
                    heights[i, j] = Generate(i, j);
            }
            heights = normalizePerlin(heights, new Vector2(size, (int)(size / frequency)));
            td.heightmapResolution = (int)(size / frequency);
            td.splatPrototypes = s.ToArray();
            td.SetHeights(0, 0, heights);
            var terrain = Terrain.CreateTerrainGameObject(td);
            terrain.tag = "Terrain";
			terrainQueue.Add(terrain);
        }
        {
            size = (int)(terrainQueue.First().GetComponent<Terrain>()).terrainData.size.x;
            //Debug.Log(size);
            float x, y = 0;
            float width = size;
            float heights = size;
            x = player.transform.position.x - width;
            for (int i = 0; i < terrainQueue.Count; i++)
            {
                ...
                x += width;
                terrainQueue.ElementAt(i).transform.position = new Vector3(x, 0, y);
               
            }
        }
        terrainQueue = GameObject.FindGameObjectsWithTag("Terrain").ToList();

    }
	
	// Update is called once per frame
	void Update () {
        foreach (var t in terrainQueue)
        {
            terrainQ.Enqueue(t);
        }
        ...
        if (!flag)
        {
            try
            {
                Debug.Log(terrainQ.Count);
                var t2 = terrainQ.Dequeue();

                if (terrainQueue.Count > 8)
                    ReorderTerrains(t2);
                else
                    GenerateNewTerrain();
    ...
    void ReorderTerrains(GameObject t)
    {
        ...
        t1.transform.position = new Vector3(theCube.transform.position.x - t1.terrainData.size.x, 0, theCube.transform.position.z - Vector3.Distance(this.transform.position, theCube.transform.position));
    }
    ...
    void Order()
    {
        ...
        for (int i = 0; i < terrainQueue.Count; i++)
        {
            ...
            x += width;
            terrainQueue.ElementAt(i).transform.position = new Vector3(x, 0, y);
            Debug.Log(terrainQueue.ElementAt(i).transform.position);
        }
    }
